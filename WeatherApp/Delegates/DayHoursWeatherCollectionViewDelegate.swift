...
class DayHoursWeatherCollectionViewDelegate: NSObject,
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        guard let weatherViewModel = weatherViewModel,
              let day = weatherViewModel.getDay(from: Shared.shared.sharedSelectedDayIndex ?? 0) else {
            return 0
        }
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        let identifier = DayWeatherCollectionViewCell.identifier
        let dequeueReusableCell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath)
        guard let cell = dequeueReusableCell as? DayWeatherCollectionViewCell else {
            return UICollectionViewCell()
        }
        if let weatherViewModel = weatherViewModel,
           let day = weatherViewModel.getDay(from: Shared.shared.sharedSelectedDayIndex ?? 0) {

...
class LocationManager: NSObject {
    ...
    func getCurrentLocation() -> (latitude: Double, longitude: Double) {
        manager.startUpdatingLocation()
        if let longitude = manager.location?.coordinate.longitude, let latitude = manager.location?.coordinate.latitude {
    ...
    func locationManagerDidChangeAuthorization(_ manager: CLLocationManager) {
        switch manager.authorizationStatus {
            case .denied, .notDetermined, .restricted:
                NotificationCenter.default.post(name: .locationNotification, object: nil, userInfo: ["authorizationStatus": manager.authorizationStatus])
                return
            default:
                manager.startUpdatingLocation()
                NotificationCenter.default.post(name: .locationNotification, object: nil, userInfo: ["authorizationStatus": manager.authorizationStatus])
